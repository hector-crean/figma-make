...
// Types
type BentoItem<T = Record<string, never>> = T & {
  id: string;
  icon: string;
  color: string;
...
const BentoContext = createContext<BentoContextValue<unknown> | null>(null);
...
function BentoBox<T>({ children, className }: BentoBoxProps<T>) {
  ...
  return (
    <BentoContext.Provider
      value={{
        selectedItem: selectedItem as BentoItem<unknown> | null,
        setSelectedItem: setSelectedItem as (item: BentoItem<unknown> | null) => void,
        hoveredItem,
        setHoveredItem,
      }}
    >
...
function BentoCard<T>({ item, index, children, className }: BentoCardProps<T>) {
  const { setSelectedItem, setHoveredItem } = useBentoContext<T>();
...
// Export everything
export {
  BentoBox, BentoCard, BentoGrid, BentoModal,
  DefaultBentoCard,
  DefaultBentoModal,
  useBentoContext
};
